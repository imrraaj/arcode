...
import { discoverSkills, discoverSkillsTool, loadSkillTool, sdbx } from "./tools/skill";
...
export function App() {
  ...
  const askUserApproval = (toolName: string, args: any): Promise<boolean> =>
    new Promise((resolve) => setPendingApproval({ toolName, args, resolve }));
    ...
    async (prompt: string) => {
      ...
      try {
        while (true) {
          ...
          const approvalRequests = content.filter((p) => p.type === "tool-approval-request");
          ...
          for (const req of approvalRequests) {
            ...
            approvals.push({ type: "tool-approval-response", approvalId: r.approvalId, approved });
          }
      ...
      {showPalette && (
        ...
        >
          <CommandPalette
            ...
            onChangeModel={(model: string) => setSelectedModel(model)}
            onClearHistory={() => {
              setMessages([]);
              setInput("");
              setCursor(0);
